...
// ИКОНКИ (Оставили только те, что нужны в App, остальные уехали в Header)
      ...
      <div className="relative z-10 h-full flex flex-col">
        ...
        <main className="relative z-0 pb-24 md:pb-20 flex-1">
          ...
          {view === 'reactor' && selectedModule && (
            <Reactor module={selectedModule} onBack={handleBackToModules} onRequestAuth={() => setShowAuthModal(true)} />
          )}

          {user && view === 'pvp' && <PvPMode onBack={handleBackToMap} />}
          
          {user && view === 'tournament_lobby' && activeTournamentId && (
            <TournamentLobby tournamentId={activeTournamentId} onBattleStart={() => setView('pvp')} />
          )}
        </main>
      </div>
