...
export function PlayerCard({ isOpponent, name, mmr, rank, winRate, skin = 'default', stage }: PlayerCardProps) {
  ...
  // Анимация входа/выхода (только сдвиг и прозрачность)
  let transformClass = '';
  ...
  // Стили скинов (CSS вместо Canvas)
  ...
  const premiumGlow = isPremium ? (isOpponent ? 'shadow-[0_0_30px_rgba(239,68,68,0.4)]' : 'shadow-[0_0_30px_rgba(6,182,212,0.4)]') : '';
  ...
  return (
    <div 
      className={`
        ...
        transition-all duration-500 ease-out ${transformClass} ${premiumGlow}
    ...
    >
      ...
      {/* Простая декорация для Premium (блик) */}
      {isPremium && (
