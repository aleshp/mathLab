...
export function PromoPage() {
  ...
  // warActive управляет стартом анимации
  ...
  return (
    <>
      {/*
        ══ WarTrailer ВСЕГДА в DOM — монтируется один раз при загрузке страницы ══
        
        Пока playing !== 'war':
          - visibility:hidden → не виден, но ПОЛНОСТЬЮ отрендерен
          - KaTeX уже скомпилировал все уравнения
          - framer-motion уже инициализирован
          - active=false → ни один таймер не запущен, фаза = 1
        
        Когда playing === 'war':
          - visibility:visible → мгновенно появляется
          - active=true → анимация стартует с нуля без лагов

        visibility:hidden (в отличие от display:none) сохраняет layout
        и не вызывает повторный рендер при переключении.
      */}
      <div
        aria-hidden={playing !== 'war'}
        style={{
          visibility: playing === 'war' ? 'visible' : 'hidden',
          // Когда скрыт — убираем из stacking context чтобы не перекрывал UI
          position: 'fixed',
          inset: 0,
          zIndex: playing === 'war' ? 9999 : -1,
        }}
      >
        <WarTrailer
          active={warActive}
      ...
      {/* CinematicTrailer — монтируется только по клику, там нет тяжёлого KaTeX */}
      {playing === 'cinematic' && (
        <CinematicTrailer
      ...
      {/* Основная страница — скрывается когда играет war-трейлер */}
      <div
        style={{ visibility: playing === 'war' ? 'hidden' : 'visible' }}
        className="min-h-screen bg-slate-950 text-white p-4 md:p-8 relative overflow-y-auto"
      >
              ...
              return (
                <div
                  ...
                  onClick={() => {
                    ...
                    if (promo.id === 'war') {
                      setPlaying('war');
                      // Небольшой delay чтобы visibility успел применится
                      // до старта анимации — браузер делает один paint
                      requestAnimationFrame(() => {
                        requestAnimationFrame(() => setWarActive(true));
                      });
                    } else {
                      setPlaying(promo.id);
                    }
                ...
                >
                  {!promo.disabled && (
                    <div
                      className={`absolute inset-0 bg-gradient-to-br ${promo.style.glow} rounded-3xl opacity-0 group-hover:opacity-100 transition-opacity duration-500 blur-xl`}
                    />
                  )}
                  <div className="relative h-full bg-slate-900/90 backdrop-blur-md border border-slate-800 rounded-[1.4rem] p-6 md:p-8 flex flex-col justify-between overflow-hidden z-10 shadow-2xl">
                    <div className="mb-8 relative z-10">
                      <div
                        className={`w-16 h-16 rounded-2xl flex items-center justify-center border mb-6 transition-transform duration-500 group-hover:scale-110 group-hover:rotate-3 ${promo.style.iconBg}`}
                      >
                    ...
                    <div className="flex items-center justify-between mt-auto pt-6 border-t border-slate-800/80 relative z-10">
                      ...
                      {!promo.disabled && (
                        <div
                          className={`flex items-center gap-2 font-bold text-sm transition-transform duration-300 group-hover:translate-x-2 ${promo.style.text}`}
                        >
