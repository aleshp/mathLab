...
export function Dashboard({ onClose, onOpenLegal }: DashboardProps) {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on('postgres_changes', {
        event: 'UPDATE', schema: 'public', table: 'teacher_requests',
  ...
  async function checkTeacherRequest() {
    ...
    const { data } = await supabase
      ...
      .limit(1)
      .maybeSingle();
    setTeacherRequestStatus(data ? (data.status as any) : 'none');
  }
  ...
  const visibleLogs = logsExpanded
    ? recentExperiments
    : recentExperiments.slice(0, LOGS_PREVIEW);
                ...
                <div className="flex flex-col md:flex-row gap-6 items-center md:items-start relative z-10">
                  ...
                  <div className="flex-1 text-center md:text-left">
                    ...
                    <div className="flex flex-wrap gap-3 mt-6 justify-center md:justify-start">
                      ...
                      <a
                        href="/pricing"
                        className="px-4 py-2 bg-emerald-600/20 hover:bg-emerald-600/30 text-emerald-400 border border-emerald-500/30 rounded-lg transition-all font-bold text-sm flex items-center gap-2"
                      >
                      ...
                      <button
                        ...
                        className="px-4 py-2 bg-blue-600/20 hover:bg-blue-600/30 text-blue-400 border border-blue-500/30 rounded-lg transition-all font-bold text-sm flex items-center gap-2"
                      >
                  ...
                  <div className="flex items-center gap-2 mb-4 px-1">
                    ...
                    {recentExperiments.length > 0 && (
                      <span className="ml-auto text-[10px] font-mono text-slate-500 tabular-nums">
                        {logsExpanded ? recentExperiments.length : Math.min(LOGS_PREVIEW, recentExperiments.length)}/{recentExperiments.length}
                      </span>
                    )}
                  </div>
                  <div className="bg-slate-900/50 border border-slate-800 rounded-2xl p-4 min-h-[200px]">
                    {recentExperiments.length > 0 ? (
                      <>
                            ...
                            >
                              ...
                              <div className="flex-1 min-w-0">
                                ...
                                <div className="text-[10px] text-slate-500">
                                  {exp.time_spent}с • {formatDate(exp.attempted_at)}
                                </div>
                              </div>
                              <span className={`text-[10px] font-bold shrink-0 ${exp.correct ? 'text-emerald-500' : 'text-red-500'}`}>
                                {exp.correct ? '+XP' : '—'}
                              </span>
                            </div>
                          ))}
                        ...
                        {recentExperiments.length > LOGS_PREVIEW && (
                          <button
                            onClick={() => setLogsExpanded(v => !v)}
                            className="mt-3 w-full flex items-center justify-center gap-2 py-2.5 rounded-xl bg-slate-800/60 hover:bg-slate-800 border border-slate-700/50 hover:border-slate-600 text-slate-400 hover:text-slate-200 text-xs font-semibold tracking-wide transition-all"
                          >
                            {logsExpanded
                              ? <><ChevronUp className="w-3.5 h-3.5" /> Скрыть</>
                              : <><ChevronDown className="w-3.5 h-3.5" /> Показать ещё {recentExperiments.length - LOGS_PREVIEW}</>}
                          </button>
                        )}
                      </>
                    ) : (
              ...
              <div className="bg-slate-800/50 p-6 rounded-2xl border border-dashed border-slate-700 text-center mt-8">
                ...
                <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
                  <a
                    ...
                    className="inline-flex items-center gap-2 px-6 py-3 bg-slate-800 hover:bg-slate-700 border border-slate-600 rounded-xl text-white font-bold transition-all"
                  >
                    <Mail className="w-4 h-4" /> support@mathlabpvp.org
                  </a>
                  <a
                    href="/pricing"
                    className="inline-flex items-center gap-2 px-6 py-3 bg-gradient-to-r from-amber-500 to-orange-600 hover:brightness-110 rounded-xl text-white font-bold transition-all shadow-lg shadow-orange-900/20"
                  >
        ...
        <div className="mt-12 pt-8 border-t border-slate-800 shrink-0">
          <div className="flex flex-wrap justify-center gap-x-8 gap-y-4">
            <button
              ...
              className="text-xs text-slate-500 hover:text-cyan-400 transition-colors flex items-center gap-1.5 uppercase font-bold tracking-wider"
            >
          ...
          <div className="flex justify-center mt-6 mb-4">
            <a
              href="https://www.producthunt.com/products/mathlab-pvp?embed=true&utm_source=badge-featured&utm_medium=badge&utm_campaign=badge-mathlab-pvp"
              target="_blank"
